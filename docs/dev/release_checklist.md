...
This guide uses `3.0.1` as the new version in its examples, so change it to match the version you bumped to in the previous step! Every. single. time. you. copy/paste commands :)

First, create a release branch off of `develop` (`git switch -c release-3.0.1 develop`).
...
Commit `git commit -m "Release v3.0.1"` and `git push` the staged changes.

### Submit Release Pull Request

Submit a pull request titled `Release v3.0.1` to merge your release branch into `main`. Copy the documented release notes into the pull request's body.
...
- **Tag:** Input current version (e.g. `v3.0.1`) and select `Create new tag: v3.0.1 on publish`
- **Target:** `main`
- **Title:** Version and date (e.g. `v3.0.1 - 2024-04-02`)

Click "Generate Release Notes" and edit the auto-generated content as follows:

- Change the entries generated by GitHub to only the usernames of the contributors. e.g. `* Updated dockerfile by @nautobot_user in https://github.com/jifox/nautobot-app-livedata.git/pull/123` -> `* @nautobot_user`.
...
**Full Changelog**: https://github.com/jifox/nautobot-app-livedata.git/compare/v1.4.1...v3.0.1
```
...
Create a new branch from `main` called `release-3.0.1-to-develop` and use `poetry version prepatch` to bump the development version to the next release.

For example, if you just released `v3.0.1`:

```no-highlight
> git switch -c release-3.0.1-to-develop main
Switched to a new branch 'release-3.0.1-to-develop'

> poetry version prepatch
Bumping version from 3.0.1 to 3.0.1a1
...
Open a new PR from `release-3.0.1-to-develop` against `develop`, wait for CI to pass, and merge it.

### Final checks

At this stage, the CI should be running or finished for the `v3.0.1` tag and a package successfully published to PyPI and added into the GitHub Release. Double check that's the case.
